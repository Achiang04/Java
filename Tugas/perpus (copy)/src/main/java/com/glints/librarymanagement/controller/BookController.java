...
public class BookController {
    ...
    public ResponseEntity<?> getAllBook() {
        ...
        for (Book books : book) {
            response.add(new BookPayload(
                    books.getBook_id(),
                    books.getAuthor().getFirstname(),
                    books.getTitle(),
                    books.getYear(),
    ...
    public ResponseEntity<?> getBookById(@PathVariable("id") Integer id){
        ...
        if (bookExist == null){
            return new ResponseEntity<ErrorResponse>(new ErrorResponse(
                    "Book Not Found",
                    "Book yang anda masukkan tidak terdaftar"
            ), HttpStatus.BAD_REQUEST);
        }

        Book book = bookRepo.findById(id).orElse(null);
        BookPayload products = new BookPayload(
                book.getBook_id(),
    ...
    public ResponseEntity createBook(@RequestBody BookPayload payload) {
        ...
        if (author == null){
            return new ResponseEntity<ErrorResponse>(new ErrorResponse(
                    "Author Not Found",
                    "Author yang anda masukkan tidak terdaftar"
            ), HttpStatus.BAD_REQUEST);
        }
        ...
        if (publisher == null){
            return new ResponseEntity<ErrorResponse>(new ErrorResponse(
                    "Publisher Not Found",
                    "Publisher yang anda masukkan tidak terdaftar"
            ), HttpStatus.BAD_REQUEST);
        }

        try {
            Book newBook = new Book(author, payload.getTitle(), payload.getYear(), publisher, payload.getCategory(), payload.getQuantity());
            bookRepo.save(newBook);
            payload.setBook_id(newBook.getBook_id());
        } catch(Exception e) {
            return new ResponseEntity<ErrorResponse>(new ErrorResponse(
                    e.getMessage(),
                    "maaf request anda tidak dapat diproses"
